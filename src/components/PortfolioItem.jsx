  ...
  return (
    ...
    <div className="w-full p-4">
        ...
        <div className="flex flex-wrap gap-2 pt-2 flex-row items-center justify-start text-xs md:text-sm">
            {demoURL && <a className="inline-block px-2 py-1 font-semibold border-2 border-slate-800 rounded-md bg-gradient-to-r from-indigo-400 to-pink-500 drop-shadow-md hover:stroke-white" target="blank" href={demoURL}>Demo</a>}
            {gitURL && <a className="inline-block px-2 py-1 font-semibold border-2 border-slate-800 rounded-md bg-gradient-to-r from-pink-500 to-indigo-400 drop-shadow-md hover:stroke-white" target="blank" href={gitURL}>GitHub</a>}
            </div>
    </div>
    </div>
  )
